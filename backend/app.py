...
from reportlab.platypus import SimpleDocTemplate, Image as RLImage, Paragraph, Spacer, Table, TableStyle, HRFlowable
...
def generate_pdf():
    ...
    conn.execute("""
        INSERT INTO laudos (patient_name, patient_dob, patient_sex, study_date,
            ...
            num_images, pdf_path, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        patient_name, patient_dob, patient_sex, study_date,
        clinic, doctor, crm, exam_type, findings, conclusion,
        len(images_b64), pdf_filename, datetime.now().isoformat()
    ))
    conn.commit()
    conn.close()
...
def list_laudos():
    conn = get_db()
    rows = conn.execute(
        "SELECT id, patient_name, patient_dob, study_date, clinic, exam_type, num_images, created_at FROM laudos ORDER BY id DESC"
    ).fetchall()
    conn.close()
    return jsonify({'laudos': [dict(r) for r in rows]})
...
def download_laudo(laudo_id):
    ...
    return send_file(pdf_path, mimetype='application/pdf',
                     as_attachment=True,
                     download_name=f"laudo_{row['patient_name'].replace(' ', '_')}.pdf")


@app.route('/api/laudos/<int:laudo_id>', methods=['DELETE'])
def delete_laudo(laudo_id):
    conn = get_db()
    row = conn.execute("SELECT pdf_path FROM laudos WHERE id=?", (laudo_id,)).fetchone()
    if row:
        try:
